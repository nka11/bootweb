...
BootWeb.initSwig = function(app, cb) {
  ...
  exists(templatesDir, function(exist) {
    ...
    if (exist) {
      logger.info("Template directory " + templatesDir+ " found");
      // Should be inserted at first element of the array
      BootWeb.templatesDirs.push(templatesDir);
    }
    else {
      logger.warn(templatesDir + " doesn't exist, not mounting templates");
    }
    templatesDir = path.join(__dirname, "../lib/templates");
    exists(templatesDir, function(exist) {
      logger.info("exits default template dir " + templatesDir + " = " + _.inspect(exist));
      if (exist) {
        logger.info("Template directory " + templatesDir+ " found");
        // Should be inserted at first element of the array
        BootWeb.templatesDirs.push(templatesDir);
...
//    app.use(express.cookieSession({secret:'super secret'}));
    ...
    exists(staticsDir, function(exist) {
      ...
      if (exist) {
        ...
        logger.warn(staticsDir + " doesn't exist, not mounting statics");
      }
      staticsDir = path.join(__dirname , "../resources/static");
      logger.info("Loading default static library : " + staticsDir);
      exists(staticsDir, function(exist) {
        logger.info("exits " + staticsDir + " = " + _.inspect(exist));
        if (exist) {
          app.use('/bootweb',express['static'](staticsDir));
        }
        else {
          logger.warn(staticsDir + " doesn't exist, not mounting default statics");
        }
