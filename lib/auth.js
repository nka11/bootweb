...
authModule.users = users;
authModule.initUsers = function(callback) {
    logger.info("Starting initUsers");
    User.findOne({where:{email: "admin"}}, function(err, user) {
		//logger.info("findOne:" + _.inspect(err) + "|"+ _.inspect(user));
        if (err !== null) {
        ...
        if (user === null || user === undefined) {
            ...
            user.save(function(err) {
              ...
              bootweb.ACL.addUserRole("admin","admin",function(err, role) {
                if (typeof callback === "function") {
                  return callback(null);
                }
            });
...
authModule.anonyMw = function(req,res,next){
	if (req.user === undefined) {
        ...
        req.user = {email: "anonymous", pseudo: "anonymous", session: req.session.id};
    } else {
...
authModule.verify = function(authorization) {
	return function(req, res, next) {
		...
		logger.debug(_.inspect({authorisation:authRequested,email:req.user.email,path:path}));
...
authModule.mapUrls = function(app) {
    ...
    if (req.is('json')) {
      ...
      authModule.registerUser(req.body.email,req.body.pseudo,req.body.password,req.headers['host'],function(err, user){
        ...
        if (err == null) {
          ...
          res.statusCode = 500;
          res.send("Operation failed : " + _.inspect(err));
        }
      });
    }
    //next();
  ...
  // Améliorer les messages
    User.findOne({where:{key:req.params.key}}, function(err, user) {
      ...
      if (emailHash === req.params.emailHash) {
        ...
        user.save(function(err) {
          ...
          if (validationData[0] === "render") {
            res.send(bootweb.swig.compileFile(validationData[1]).render({user: req.user}));
          }
