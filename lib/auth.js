...
exports.verify = function(authorization) {
	return function(req, res, next) {
		...
		if (authorization === null || authorization === undefined) { // no parameter given, last part of path used as auth :
			logger.debug("Authorization param is null, splitting path : " + path);
			authorization = path.substr(path.lastIndexOf("/") + 1);
			path = path.substr(0, path.lastIndexOf("/"));
		}
		logger.debug(_.inspect({authorisation:authorization,email:req.user.email,path:path}));

		bootweb.ACL.isAuthorized(req.user.email,path, authorization, function(err, isAuth) {
			if (err !== null) {
