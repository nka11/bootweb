 ...
 */
authModule.findOrCreateUser = function(username, password, cb) {
    User.findOne({
      email: username
    }, function(err, user) {
      if (err !== null) {
      ...
      if (user === null) {
        ...
        authModule.createUser(username,username,password,cb);
      }
      else {
...
authModule.initUsers = function(callback) {
  ...
  authModule.findOrCreateUser("admin", "demo", function(err, admin) {
    bootweb.ACL.addUserRole(admin, "admin");
    authModule.findOrCreateUser("anonymous", "anonymous", function(err,anonymous){
      callback(null);
    });
  });
