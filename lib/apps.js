...
var logger = require("./bootweb").getLogger("bootweb.apps"),
  appsRegistry = {
    ...
    initApps: function(app,cb) {
      ...
      logger.info("Starting Apps loading");
      mapAppsUrls = function(err, appModule) {
        ...
        if (typeof cb === "function") {
          logger.info(registry.appsList.length + " apps loaded");
          cb();
        }
      };
      if (registry.appsList.length === 0) {
        if (typeof cb === "function") {
          logger.info("No apps to load");
          return cb();
        }
      }
      registry.appsList.forEach(function(appName) {
          registry.apps[appName].app.init(registry.apps[appName].config,mapAppsUrls);
      });
    }
  };
